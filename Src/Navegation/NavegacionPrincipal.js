...
import CategoriasStack from './Stacks/CategoriaStack'; // <-- Importa Categorias
import EspecialidadesStack from './Stacks/EspecialidadStack'; // <-- He añadido este
import OrdenesStack from './Stacks/OrdenStack'; // <-- He añadido este

// --- Tu componente de Barra Personalizada (SIN CAMBIOS) ---
function MiBarraPersonalizada({ state, descriptors, navigation, updateUserToken }) {
    // ... (El código de la barra personalizada se mantiene igual)
    return (
                ...
                return (
                    ...
                    >
                        {options.tabBarIcon && options.tabBarIcon({
                            focused: isFocused,
                            color: isFocused ? '#0D47A1' : '#757575',
                            size: 24
                        })}
                        <Text style={{ color: isFocused ? '#0D47A1' : '#757575', fontSize: 11 }}>
                            {label}
                        </Text>
                    </TouchableOpacity>
                );
...
export default function NavegacionPrincipal({ updateUserToken }) {
    return (
        ...
        >
            {/* Tus pantallas visibles (SIN CAMBIOS) */}
...
// --- Estilos para la barra (SIN CAMBIOS) ---
