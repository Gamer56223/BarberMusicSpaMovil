...
export const listarServicioSucursales = async () => {
    try {
        const response = await api.get("/Admin_servicio_sucursal/servicio_sucursal");
...
export const DetalleServicioSucursalId = async (id) => {
    try {
        const response = await api.get(`/Admin_servicio_sucursal/servicio_sucursal/${id}`);
...
export const eliminarServicioSucursal = async (id) => {
    ...
    try {
        const response = await api.delete(`/Admin_servicio_sucursal/servicio_sucursal/${id}`);
        console.log("Respuesta eliminarServicioSucursal:", response.data);
        return { success: true, message: response.data.message || "Servicio Sucursal eliminado correctamente" };
    } catch (error) {
        const errorMessage = error.response ? formatErrorMessage(error.response.data) : "Error de conexión";
...
export const crearServicioSucursal = async (data) => {
    try {
        const response = await api.post("/Admin_servicio_sucursal/servicio_sucursal", data);
...
export const editarServicioSucursal = async (id, data) => {
    try {
        const response = await api.put(`/Admin_servicio_sucursal/servicio_sucursal/${id}`, data);
