...
export const listarTransaccionPagos = async () => {
    try {
        const response = await api.get("/Payments_transacciones_pago/transacciones_pago");
...
export const DetalleTransaccionPagoId = async (id) => {
    try {
        const response = await api.get(`/Payments_transacciones_pago/transacciones_pago/${id}`);
...
export const eliminarTransaccionPago = async (id) => {
    ...
    try {
        const response = await api.delete(`/Payments_transacciones_pago/transacciones_pago/${id}`);
        console.log("Respuesta eliminarTransaccionPago:", response.data);
        return { success: true, message: response.data.message || "Transaccion Pago eliminada correctamente" };
    } catch (error) {
        const errorMessage = error.response ? formatErrorMessage(error.response.data) : "Error de conexión";
...
export const crearTransaccionPago = async (data) => {
    try {
        const response = await api.post("/Payments_transacciones_pago/transacciones_pago", data);
...
export const editarTransaccionPago = async (id, data) => {
    try {
        const response = await api.put(`/Payments_transacciones_pago/transacciones_pago/${id}`, data);
